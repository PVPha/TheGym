    ...
    @if (session()->has('login'))
        ...
        <div class="container" style="margin-top: 10%">
            <div class="d-flex justify-content-end">
                <a class="btn btn-danger" href="/news/delete/{{$data->id}}">Xóa bài viết</a>
            </div>
            <form name="addNews" action="{{url('news/update')}}" method="POST" enctype="multipart/form-data">
                @method('put')
                @csrf
                ...
                <div class="form-group d-flex">
                  ...
                  <img id="imageNew" src="/{{$data->image}}" alt="image" style="{{$data->image == '' ?'display: none':'display: block'}} ; max-width: 50rem; max-height: 25rem;">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="basic-example" name="content" rows="3">{{$data->content}}</textarea>
                </div>
                <button type="submit" class="form-control" id="submit-button" name="data-register">Cập nhật</button>
            </form>
        </div>
        @endif
    @else
